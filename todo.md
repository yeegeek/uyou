...
- [x] **评价功能**
  - [x] 提交评价（`POST /reviews`）
  - [x] 顾问评价用户（`POST /reviews/consultant`）
  - [x] 查询评价列表（`GET /reviews`）
  - [x] 评价标签管理（`GET /reviews/tags`）
  - [x] 打赏功能（`POST /reviews/tips`）
  - [x] 删除评价（`DELETE /reviews/:id`）
  - [x] 实现评价审核流程（0待审核/1显示/2审核拒绝）
  - [x] 实现评价审核通过后的评分更新
  - [x] 实现评价删除后的评分重新计算
  - [x] 实现评分更新的自动计算
  - [ ] 实现评分缓存机制（Redis缓存顾问评分，定期同步到数据库）待集成
